...
```
  ...
  (iPhone, calendar,          (Collector + Coach        (Telegram chat,
   bank, music, …)             on GCP / Docker)         CLI, future channels)
       │                             ▲
       └── encrypted overlay ────────┘
              (Pilot Protocol)
```
...
**Collector (`agent-a`)** is the warehouse and an OpenClaw LLM agent. It accepts envelopes from
sources over Pilot, dedupes by sample UUID, writes to DuckDB, and serves
read-only SQL. The ingest pipeline is fast and deterministic; the LLM
layer is available for reasoning — combining DuckDB + its own gbrain +
the peer-reviewed RAG into an opinionated answer when queried.
...
Pilot Protocol is the transport. All messages are delivered via `pilotctl send-message <target-node-id> --data '<json>'` — a single primitive. There are no application-level port numbers. The receiver classifies messages by **JSON content shape**:

| Content shape | Direction | Classified as |
|---|---|---|
| `samples` array | Source → Collector | `Envelope` (HealthKit samples + workouts + route chunks) |
| `sql` field | Coach → Collector | `Query` (read-only SQL) |
| `kind: "samples_added"` | Collector → Coach | `ChangeEvent` (after each batch commit) |
| ack JSON | Collector → Source | `Ack` (per-sample accepted/duplicate/rejected) |

Real Pilot built-in ports: 1001 (dataexchange/send-message), 7 (echo), 444 (handshake). No virtual application ports.
...
│
├── systemd: coach-proactive.timer  (every 15 min on host)
│      python -m coach proactive
│      Evaluates rule engine, writes gbrain pages, sends Telegram nudges
│
...
- **DuckDB** is single-writer. The Collector's daemon owns the lock; all
  reads must go through Pilot via a `Query` message (classified by `sql` field).
...
- **4 of 7 rule models**: cognitive_recovery_debt, burnout_cusum, circadian_drift, kalman_hrv — require 3–8 weeks of baseline data. The other 3 (sleep_regularity, autonomic_balance, sedentary_stress) are implemented and running.
